...
export const getBeerById = async (req: Request<{ id: string }>, res: Response): Promise<any> => {
  const db = await initDB();
...
export const updateBeer = async (req: Request<{ id: string }>, res: Response): Promise<any> => {
  const { name, alcohol, price, rating } = req.body;
  const db = await initDB();
  const result = await db.run(
...
export const deleteBeer = async (req: Request<{ id: string }>, res: Response): Promise<any> => {
  const db = await initDB();
